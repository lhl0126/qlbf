...
let userCookie = ($.isNode() ? process.env.sm_data : $.getdata('sm_data')) || '';
...
class UserInfo {
    ...
    async postSign() {
        let url = `https://suming123.com/user/sign`
        
        let urlObject = populateUrlObject(url,this.token)
    ...
    if(userCookie) {
        for(let userCookies of userCookie.split('@')) {
            if(userCookies) {
                ...
                //userList.push(new UserInfo(str[0],str[1]))
...
function populateUrlObject(url,form){
    let host = url.replace('//','/').split('/')[1]
    let urlObject = {
        url: url,
        headers: {},
    }
    if(form) urlObject.form = parseQueryString(form)
    return urlObject;
}
        ...
        $[method](url,async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    let msg = reconvert(data.replace(/\%u/g, "%u"))
                    showmsg(msg)
                    console.log(msg)
                }
            } catch (e) {
